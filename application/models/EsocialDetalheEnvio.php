...
class Application_Model_EsocialDetalheEnvio extends Zend_Db_Table {
    ...
    public function dadosEventosEnviado($id, $tipo) {
        ...
        $condicao = '';
        if ($tipo == 'S2240') {
            $condicao = ' AND f.fk_contrato_id = '. $_SESSION['contrato_id'] . ' ORDER BY ps.pessoa_nome ASC';
        }else{
            $condicao = ' AND ed.fk_contrato_id = '. $_SESSION['contrato_id'] . ' ORDER BY p.pessoa_nome ASC';
        }
        ...
        $sql = "SELECT 
                ...
                WHERE ed.esocial_detalhe_envio_status = 0
                    AND ed.fk_esocial_envio_tecnospeed_id = {$id}
                    {$condicao}";
